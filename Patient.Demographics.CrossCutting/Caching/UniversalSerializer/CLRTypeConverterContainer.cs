...
namespace UniversalSerializerLib3
{
	...
	{
		static ConcurrentDictionary<Type, TypeConverter> VSCache = new ConcurrentDictionary<Type, TypeConverter>();
		...
		static TypeConverter GetTypeConverterByCache(Type t)
		{
			...
			CLRTypeConverterContainer.VSCache.TryAdd(t, vs);
			return vs;
		}
		...
		public bool IsValidType(System.Type type)
		{
			...
			if (!IsValidTypeCache.TryGetValue(type, out ret))
			{
				...
				IsValidTypeCache.TryAdd(type, ret);
			}
			return ret;
		}
		static ConcurrentDictionary<Type, bool> IsValidTypeCache = new ConcurrentDictionary<Type, bool>();
			...
			Type SourceObjectType, Type SerializationType, object SerializedObject)
		{
			...
			if (!GenericContainersTypeCache.TryGetValue(k, out g))
			{
				...
				GenericContainersTypeCache.TryAdd(k, g);
			}
		...
		static ConcurrentDictionary<KeyValuePair<Type, Type>, Type> GenericContainersTypeCache
