...
namespace UniversalSerializerLib3.TypeManagement
{
	...
	internal class L3TypeManagerCollection
	{
		...
		readonly ConcurrentDictionary<Type, L3TypeManager> TypeManagerCache = new ConcurrentDictionary<Type, L3TypeManager>();
		readonly ConcurrentDictionary<L2TypeManager, L3TypeManager> l2TypeManagerCache = new ConcurrentDictionary<L2TypeManager, L3TypeManager>();
	        ...
	        bool ReserveSiteFirst)
	    {
	        ...
	        tm = new L3TypeManager(this, l2TypeManager, index, this.TypeIndexationStartForThisVersion,
	            ...
	            && !l2TypeManager.IsAPrimitiveType)
	        {
	            ...
	            serializationFormatter.AddAnObject(
	                ref
	                serializationFormatter.channelInfos[
	                    (int) SerializationFormatter.ChannelNumber.TypeDescriptorsChannel],
	                descr, null,
	        ...
	        this.l2TypeManagerCache.TryAdd(l2TypeManager, tm);
	        this.TypeManagerCache.TryAdd(l2TypeManager.L1TypeManager.type, tm);
		...
		{
			this.l2TypeManagerCache.TryAdd(tm.l2TypeManager, tm);
			this.TypeManagerCache.TryAdd(type, tm);
		...
		internal L3TypeManager AddTypeManager(SerializationTypeDescriptor Descriptor, bool NeedsTheValue = true)
		{
		    ...
		    var tm = this.l2TypeManagerCollection.GetTypeManager(type, NeedsTheValue, true);
			return this.AddTypeManager(tm,null, NeedsTheValue, true);
	...
	internal abstract class L2TypeManager
	{
		...
		internal L2TypeManager(
			...
			bool ReserveSiteFirst)
		{
			l2typeManagerCollection.TypeManagerCache.TryAdd(type, this);
	...
	internal class L2TypeManagerCollection
	{
		...
		internal readonly ConcurrentDictionary<Type, L2TypeManager> TypeManagerCache = new ConcurrentDictionary<Type, L2TypeManager>();
		...
		internal bool CanThisTypeBeSerialized(Type type)
		{
			...
			if (!this._ThisTypeCanBeSerializedCache.TryGetValue(type, out ret))
			{
				...
				this._ThisTypeCanBeSerializedCache.TryAdd(type, ret);
			}
			return ret;
		}
		ConcurrentDictionary<Type, bool> _ThisTypeCanBeSerializedCache = new ConcurrentDictionary<Type, bool>();
			...
			bool ReserveSiteFirst)
		{
			...
			L2TypeManager tm;
            if(BeingAnalysed.Contains(type))
                Thread.Sleep(150);
            if (!this.TypeManagerCache.TryGetValue(type, out tm))
			{
				if (this.BeingAnalysed.Contains(type))
			        ...
			        {
			            return L2GenericTypeManager<int>.CreateGenericTypeManager(
			                this,
			                type, this.customModifiers,
			                ReserveSiteFirst);
			            //return null; // for CheckTypeManager().
...
(Func<object>)Delegate.CreateDelegate(FuncObjectType, StructureConstructorMethod.MakeGenericMethod(type)));
	...
	{
		static ConcurrentDictionary<Type, L1TypeManager> TypeDescriptions = new ConcurrentDictionary<Type, L1TypeManager>();
		...
		internal static L1TypeManager GetTypeDescription(Type type)
		{
			...
			if (!TypeDescriptions.TryGetValue(type, out td))
			{
				...
				TypeDescriptions.TryAdd(type, td);
			}
			return td;
		}
	}
