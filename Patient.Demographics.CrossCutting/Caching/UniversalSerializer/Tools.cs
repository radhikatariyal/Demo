...
, "value"
		...
		{ 
			...
			var cast_param = Expression.Convert(param_value, pi.PropertyType);
			var assign = 
				Expression.Assign(Property, cast_param);
			var lambda = Expression.Lambda<ValueTypeObjectMemberSetterDelegate<T>>(assign, param_struct, param_value);
			return lambda.Compile();
		}
		...
		{
			ConstructorInfo ci = type.IsPublic ?
			type.GetConstructor(
 ...
 pars);
		...
		internal static Type FindDerivedToThisType(this Type TheObjectType, Type SearchedType)
		{
			...
			if (!_FindDerivedToThisTypeCache.TryGetValue(searched, out ret))
			{
				...
				_FindDerivedToThisTypeCache.TryAdd(searched, ret);
			}
			return ret;
		}
		...
		static ConcurrentDictionary<KeyValuePair<Type, Type>, Type> _FindDerivedToThisTypeCache
			= new ConcurrentDictionary<KeyValuePair<Type, Type>, Type>();
		...
		public static Type GetTypeFromFullName(string TypeFullName)
		{
			...
			if (!_GetTypeFromFullNameCache.TryGetValue(TypeFullName, out ret))
			{
				...
				_GetTypeFromFullNameCache.TryAdd(TypeFullName, ret);
			}
			return ret;
		}
		...
		static ConcurrentDictionary<string, Type> _GetTypeFromFullNameCache = new ConcurrentDictionary<string, Type>( );
 ...
 && (comparableItem.GUID == value.GUID)
	...
	/// </summary>
	public class SimpleLazy<T>:Lazy<T>
	{
	    public SimpleLazy(Func<T> valueFactory):base(valueFactory)
	    {
	    }
...
namespace UniversalSerializerLib3.TypeTools
{
	...
	internal static class Types
	{
		...
		{
			var list = new ConcurrentDictionary<string, FieldInfo>();
		...
		{
			object[] att = mi.GetCustomAttributes(typeof(ForceSerializeAttribute), false);
		...
		{
			object[] att = mi.GetCustomAttributes(typeof(ForceNotSerializeAttribute), false);
			return (att != null && att.Length > 0);
		}
...
namespace UniversalSerializerLib3.DataTools
{
	...
	internal class FrequencyOrderedLimitedSizeDict<TKey, TValue>:ConcurrentDictionary<TKey,TValue>
		where TKey : class
	{
		readonly TKey[] Keys;
