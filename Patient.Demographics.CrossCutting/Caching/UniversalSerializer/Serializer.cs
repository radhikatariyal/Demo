...
namespace UniversalSerializerLib3
{
	...
	internal abstract partial class SerializationFormatter
	{
			...
			ref L3TypeManager typeManager)
		{
			...
			if (obj2 == null)
			{
				typeManager.l2TypeManager.Container = null; // We do not use this container anymore.
				return obj;
			}
		...
		/// </summary>
		/// <param name="data"></param>
		internal void SubSerialize(object data)
		{
 ...
 tm.TypeIndex,
			...
			string NameToWrite = null)
		{
			...
			L3TypeManager typeManager =
				...
				collectionTypeManager : this.l3typeManagerCollection.GetTypeManager(collectionType, this, true, false);
			int collectionTypeNumber =typeManager.TypeIndex;

			if (iEnumerable.IsEmpty())
			{
				// Optimisation: we only write an empty object as a collection.
				this.ChannelAddNull(ref channelInfos);
				return;
			}
...
, collectionType
			...
			else // structures or sealed classes can not inherit therefore can be serialized faster:
			{
#if !DEBUG
